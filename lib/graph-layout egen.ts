...
export function computeGraphLayout(
  ...
  nodesByLayer.forEach((layerNodes, layer) => {
    layerNodes.sort((a, b) => (pathlengths.get(a.id) || 0) - (pathlengths.get(b.id) || 0))
  });
  // Sort and put biggest values in the midde
  nodesByLayer.forEach((layerNodes, layer) => {
    layerNodes.sort((a, b) => {
        const valA = pathlengths.get(a.id) || 0;
        const valB = pathlengths.get(b.id) || 0;
        
        // Sort by distance from middle (largest values should be near center)
        const length = layerNodes.length;
...
export function computeGraphLayoutOld(
  nodes: GraphNode[],
  edges: GraphEdge[],
  canvasCrossAxisSize: number,
  orientation: 'horizontal' | 'vertical' = 'horizontal',
  ...
  console.log('Post-order positions:', positions)
  ...
  console.log('Pre-order positions:', positions)
